...
run_step3() {
...

  {
    ...
    echo "## 출력 분량 규칙 (절대 필수)"
    echo ""
    echo "- Round 1 분석 결과의 분량 이상으로 작성하세요. 요약으로 대체하는 것은 절대 금지입니다."
    echo "- 이전 분석의 모든 항목을 순회하면서 각각에 대해 보강/수정/유지를 명시하세요."
    echo "- '반영 완료'라는 선언만으로는 부족합니다. 구체적 내용을 본문에 포함하세요."
    echo "- 비판 검토에서 지적된 각 항목에 대해 어떻게 교정했는지 원문 인용과 함께 서술하세요."
    echo "- 필수 출력 형식의 모든 섹션을 빠짐없이 작성하세요."
    echo ""
    echo "## 입력 자료"
    echo ""
    echo "--- 원본 입력 ---"
    echo "$INPUT_CONTENT"
    echo ""
    echo "--- [핵심] Round 1 $(provider_label "$REVIEWER") 비판적 검토 결과 (이 내용을 최우선으로 반영하세요) ---"
    cat "$r1_review"
    echo ""
    echo "--- Round 1 $(provider_label "$PROVIDER") 분석 결과 ---"
    cat "$r1_analysis"
